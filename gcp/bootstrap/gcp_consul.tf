resource "tfe_workspace" "gcp_consul" {
  name                      = "${var.name}-gcp-consul"
  organization              = var.tfc_organization
  project_id                = data.tfe_project.project.id
  description               = "Workspace for consul that run on Google Cloud"
  working_directory         = "gcp/consul"
...
resource "tfe_workspace_variable_set" "google_cloud_consul" {
  workspace_id    = tfe_workspace.gcp_consul.id
