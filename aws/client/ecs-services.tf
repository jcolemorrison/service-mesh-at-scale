module "consul_acl_controller" {
  ...
  consul_server_http_addr = join("", ["http://", jsondecode(base64decode(data.hcp_consul_cluster.aws.consul_config_file))["retry_join"], ":8500"])
...
module "mesh_gateway" {
  ...
  acls = true
  consul_http_addr = join("", ["http://", jsondecode(base64decode(data.hcp_consul_cluster.aws.consul_config_file))["retry_join"], ":8500"])
