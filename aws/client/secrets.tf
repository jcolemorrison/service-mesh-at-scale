...
resource "aws_secretsmanager_secret_version" "consul_gossip_key" {
  ...
  secret_string = jsondecode(base64decode(data.hcp_consul_cluster.aws.consul_config_file))["encrypt"]
}
...
resource "aws_secretsmanager_secret_version" "consul_root_ca_cert" {
  ...
  secret_string = base64decode(data.hcp_consul_cluster.aws.consul_ca_file)
}